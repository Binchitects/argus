...
large enough that the numbers in `docs/argus/index-measurements.md` mean something.

    python scripts/test-gitlab/seed_corpus.py --tier baseline
    python scripts/test-gitlab/seed_corpus.py --tier scale
    python scripts/test-gitlab/seed_corpus.py --list
...
#: docs/argus/index-measurements.md. They deliberately share header basenames
...
def admin_token() -> str:
    ...
    if not seeded.exists():
        sys.exit("seeded.json missing -- run scripts/test-gitlab/seed.py first")
...
def main() -> int:
    ...
    print("now run: python -m argus.cli index --config scripts/test-gitlab/work/config.yaml")
    return 0
