...
Everything here runs from `stack/`. Each `make` target has a script in
`stack/scripts/`, in Bash and in PowerShell.
...
```bash
cp backups/argus-<time>/env stack/.env
cd stack && docker compose up -d postgres argus && docker compose stop argus
...
Put a certificate and key in `stack/config/argus/tls/`, set
