...
every setting an admin can change: 101 of them in 13 groups. Each is typed and
...
| **.env** | `deploy/.env` | When you apply it on the host. Examples: the model, the engine and hardware, image generation, prices, credit defaults, Argus's GitLab connection, profiles, retention and backup. |
