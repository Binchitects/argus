...
`stack/.env` is the whole deployment. Start from a sample in
`stack/env-samples/`; each is a measured setup for one model on one card. This
...
one-liner in `stack/README.md` generates them all.
