...
| `postgres-data` | the app's people, 2FA and audit log (`llmapp`), LiteLLM's keys/spend, **and Langfuse's traces** | no — this is the accounts, billing and audit history |
...
(`app/src/Llm.Api/Oidc/ForwardAuth.cs`): infrastructure hosts are admins only,
