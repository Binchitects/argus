...
> **History.** Measured with the evaluation harness of the Python implementation, which
> has since been retired; the retrieval logic it measured is unchanged in the .NET backend.

The Step 0 run from `roadmap.md`. Production GitLab was not reachable, so
...
```bash
python scripts/test-gitlab/seed_corpus.py --tier baseline   # ~50 s
python -m argus.cli index --config scripts/test-gitlab/work/config.yaml
```
...
`scripts/test-gitlab/seed_corpus.py --tier scale`.
