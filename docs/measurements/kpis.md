...
Rebuild with `scripts/test-gitlab/seed_corpus.py --tier baseline`, which pins
...
`index-measurements.md` records by how much.
...
miss is recorded with its cause in `index-measurements.md` rather than
tuned away.
...
(`index-measurements.md` has the full run).
