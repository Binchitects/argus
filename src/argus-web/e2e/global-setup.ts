...
export default async function globalSetup() {
  ...
  const bin = process.env.ARGUS_BIN ?? join(repo, "src/Argus/bin/Release/net10.0/argus");
  if (!existsSync(bin)) throw new Error(`no backend at ${bin}: run 'dotnet build LlmService.slnx -c Release' first`);
  ...
  await promisify(execFile)(bin, ["pack", "build", "--source", "python", "--work-dir", join(repo, "tests/fixtures/argus/packs/python"),
