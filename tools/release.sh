...
#
#   ./tools/release.sh 2.0.0              build, verify, push to ghcr.io
#   ./tools/release.sh --offline 2.0.0    build, verify, write dist/
#
...
# the layout deploy/scripts/package.sh already looks for when it builds a
...
if [[ $OFFLINE -eq 1 ]]; then
  ...
  echo
  echo "  Or let deploy/scripts/package.sh carry them for you: it picks up"
