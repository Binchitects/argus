...
| Real certificates | `tls-init` generates a self-signed certificate — fine internally, not for the public internet |
...
| `bootstrap` | `.env`, all secrets, proxy basic-auth (the TLS certificate is generated by `tls-init` on first `up`) |
...
HTTPS works immediately with a self-signed certificate generated on the first
`up`. Each browser warns once; accept it and it stays accepted.
...
| `gen-proxy-auth` | Basic-auth file for Traefik |
| `domain-check.sh` | Proves the running stack answers for `LLM_DOMAIN`: certificate, routes, SSO |
| `functional-test.py` | Every person-facing flow for real: panel, keys, budgets, rotation, SSO roles, attribution |
| `multiuser-bench.py` | What two people using the stack at once each get |
| `qwen-code-realworld.py` | Qwen Code on a large codebase, scored against grep-established answers |
...
**TLS errors from curl or PowerShell.** The self-signed certificate has no CRL, and
Windows treats "revocation unknown" as fatal. Use
`--ssl-no-revoke --cacert config/traefik/certs/tls.crt`.
...
docs/AUTHENTICATION.md   SSO design, team management, turning 2FA back on
