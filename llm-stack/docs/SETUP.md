...
It orchestrates rather than reimplements. `bootstrap.sh`, `gen-proxy-auth`,
...
So the domain now lives in `.env` and **nowhere else**. Change `LLM_DOMAIN` and
run `docker compose up -d`; that is the whole procedure:

- **Traefik routes** and every service URL interpolate it from `.env`.
- **The TLS certificate** is regenerated by `tls-init` when the domain changes.
- **Authelia** loads `configuration.template.yml` directly and expands
  `{{ env "LLM_DOMAIN" }}` itself at startup (its `template` config filter), and
  the OIDC redirect URIs in `clients.yml` are written the same way. Nothing is
  rendered, so nothing can go stale.

`scripts/domain-check.sh` proves it against the running stack: certificate,
every route, the OIDC issuer, Grafana and Open WebUI SSO redirects, in-network
resolution, and (with `--old <previous>`) that the old domain no longer routes.
Verified on `llm.localhost`, `ai.localhost` and `llm.example.test`.

One file is still rendered by `gen-auth.sh`, because Homepage cannot read the
...
`setup.sh` is safe to re-run: the TLS certificate, the OIDC signing key, `users.yml`
and every secret already in `.env` are kept.
...
| `LLAMACPP_PARALLEL` | request slots, i.e. people served at the same time. With `--kv-unified` in `LLAMACPP_EXTRA_ARGS` (shipped) all slots share ONE `CONTEXT`-sized pool, so each person can still reach the full window; without it each slot gets `CONTEXT/PARALLEL`. |
...
**1. Accept the certificate warning once.** HTTPS uses a self-signed certificate that the
`tls-init` service generates on the first `docker compose up` and keeps on a
volume. Each browser warns once; accept it and it stays accepted, because the
certificate does not change on restart. API clients either skip verification or
trust `config/traefik/certs/tls.crt`, which `tls-init` writes for you.
...
`LLM_DOMAIN` and `TRAEFIK_HTTPS_PORT` and trusts the stack's own certificate. Override
