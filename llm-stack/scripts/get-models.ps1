<#
.SYNOPSIS
    PowerShell wrapper for scripts/get-models.sh.
...
if (-not $bash) {
    Write-Host 'bash not found. Install Git for Windows (it ships bash).' -ForegroundColor Red
    exit 1
}

# Git Bash wants an MSYS path (/e/foo), not a Windows one (E:/foo).
$unix = '/' + $root.Substring(0,1).ToLower() + $root.Substring(2).Replace('\', '/')
$script = "$unix/scripts/get-models.sh"
& $bash "$script" @Rest
exit $LASTEXITCODE
