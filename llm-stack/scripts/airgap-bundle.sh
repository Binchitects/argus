...
[ -n "$NAME" ] || NAME="llm-stack-airgap-$(date +%Y-%m-%d)"
...
# trying to avoid. `llm-stack/models/.gitkeep` and the certs/auth/prometheus
...
tar_excludes=(
  ...
  --exclude='./dist'
  --exclude='./deploy/test-gitlab/work'
  --exclude='./llm-stack/.env'
  --exclude='./llm-stack/.env.*'
  --exclude='./llm-stack/backups'
  --exclude='./llm-stack/config/traefik/certs/tls.crt'
  --exclude='./llm-stack/config/traefik/certs/bundle.crt'
  --exclude='./llm-stack/config/traefik/auth/users.htpasswd'
  --exclude='./llm-stack/config/authelia/clients.yml'
  --exclude='./llm-stack/config/authelia/users.yml'
  --exclude='./llm-stack/config/authelia/users.yml.bak'
  --exclude='./llm-stack/config/authelia/secrets/*'
  --exclude='./llm-stack/config/prometheus/secrets/llamacpp.token'
  --exclude='*/__pycache__'
  --exclude='*.pyc'
  --exclude='./llm-stack/models/*'
)
...
mkdir -p "$STAGE/llm-stack/config/authelia/secrets"
[ -f "$REPO_ROOT/llm-stack/models/.gitkeep" ] \
  && cp "$REPO_ROOT/llm-stack/models/.gitkeep" "$STAGE/llm-stack/models/.gitkeep"
...
airgap_env="$STAGE/llm-stack/.env.airgap"
...
say "  wrote llm-stack/.env.airgap -- $(wc -l < "$secrets_list" | tr -d ' ') secret(s) emptied, downloads disabled"
...
if [ "$WITH_ENV" = 1 ]; then
  cp "$STACK_DIR/.env" "$STAGE/llm-stack/.env"
  chmod 600 "$STAGE/llm-stack/.env"
...
* \`.git\`, and the test corpus under \`deploy/test-gitlab/work\`.
* Any **secrets** with values in them, unless this bundle was built with
  \`--with-env\` (in which case \`llm-stack/.env\` is the live one, verbatim).
