...
if [ ! -f "$OLLAMA_TAR" ]; then
  ...
  volume_source=""
  if [ -f stack/.env ]; then
    volume_source="stack/.env"
  elif [ -f stack/.env.airgap ]; then
    volume_source="stack/.env.airgap"
  fi
...
if [ ! -f stack/.env ]; then
  ...
  if [ "$CHECK_ONLY" = 1 ]; then
    ...
    warn "stack/.env does not exist yet (expected before setup)"
    warn "create it with:  cp stack/.env.airgap stack/.env && ./fill-secrets.sh"
  elif [ -f stack/.env.airgap ]; then
    die "stack/.env does not exist yet. Make it from the shipped template:
      cp stack/.env.airgap stack/.env
      ./fill-secrets.sh
    then run this again."
  else
    die "stack/.env does not exist and no .env.airgap was shipped"
  fi
else
  say "  stack/.env present"
fi
...
if [ "$env_ready" = 0 ]; then
  ...
  ( cd stack && ./scripts/preflight.sh ) || exit 1
  say ""
  say "Check complete. Nothing was changed."
  exit 0
else
  ( cd stack && ./scripts/preflight.sh ) || die "preflight failed; not starting"
fi
...
if [ "$DO_UP" = 1 ]; then
  ...
  ( cd stack && docker compose up -d --no-build )
  say ""
  ( cd stack && docker compose ps --format 'table {{.Name}}\t{{.Status}}' )
  say ""
  say "Watch the model come up:  cd stack && docker logs -f model-init"
  say "Then open https://admin.<LLM_DOMAIN> (see LLM_DOMAIN in stack/.env)."
else
  say ""
