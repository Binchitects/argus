...
mkdir -p "$PKG/scripts" "$PKG/docs" "$PKG/services" "$PKG/models" "$PKG/config"
...
# The docs live in the repository's docs/, one level up from this deployment,
# not inside deploy/ -- so they are gathered from there. Discovering that the
# hard way is why this comment exists: `cp docs/*.md` with no docs/ silently
# produced an archive containing no documentation at all, and cp's failure was
# swallowed by the `2>/dev/null` that is there for optional files.
ls "$ROOT/../docs/"*.md >/dev/null 2>&1 || { printf '  %sERROR: %s has no docs%s\n' "$red" "$ROOT/../docs" "$off"; exit 1; }
cp "$ROOT/../docs/"*.md "$PKG/docs/"
for d in "$ROOT/../docs/argus"; do
  [[ -d "$d" ]] || { printf '  %sERROR: %s is missing%s\n' "$red" "$d" "$off"; exit 1; }
  cp -r "$d" "$PKG/docs/"
done
cp services/*.yml "$PKG/services/" 2>/dev/null
cp services/*.py "$PKG/services/" 2>/dev/null
...
say "services/: $(echo $built_dirs | tr '\n' ' ')"
...
# LLM_SOURCE_DIR (..). Only tracked sources: never bin/, obj/ or node_modules/,
# and not the tests, which the target never runs.
( cd "$ROOT/.." && git ls-files src/Llm.Api src/Llm.Core src/web global.json Directory.Build.props ) \
  | grep -vE '^src/web/(e2e|ci)/' | while read -r f; do
  mkdir -p "$STAGE/$(dirname "$f")" && cp "$ROOT/../$f" "$STAGE/$f"
done
[[ -f "$STAGE/src/Llm.Api/Dockerfile" ]] || { printf '  %sERROR: the app sources are missing from the package%s\n' "$red" "$off"; rm -rf "$STAGE"; exit 1; }
...
if [[ $INCLUDE_IMAGE -eq 1 ]]; then
  ...
  # rather than in one of them: `tools/release.sh --offline 2.0.0` from the
  ...
  if [[ -n "$images_dir" ]]; then
    ...
    say "${dim}(make it with:  ./tools/release.sh --offline <version>   from the repository root)${off}"
  fi
fi
...
say "  unzip llmservice-${VERSION}.zip && cd llmservice     # src/ unpacks next to it"
