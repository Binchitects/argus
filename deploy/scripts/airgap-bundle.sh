...
# trying to avoid. `deploy/models/.gitkeep` and the certs/auth/prometheus
...
tar_excludes=(
  ...
  --exclude='./.ruff_cache'
  --exclude='./dist'
  --exclude='./tools/test-gitlab/work'
  --exclude='./deploy/.env'
  --exclude='./deploy/.env.*'
...
say "  wrote deploy/.env.airgap -- $(wc -l < "$secrets_list" | tr -d ' ') secret(s) emptied, downloads disabled"
...
* \`.git\`, and the test corpus under \`tools/test-gitlab/work\`.
* Any **secrets** with values in them, unless this bundle was built with
  \`--with-env\` (in which case \`deploy/.env\` is the live one, verbatim).
