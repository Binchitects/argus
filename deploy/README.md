...
is `cp deploy/.env.airgap deploy/.env && ./fill-secrets.sh && ./load.sh --up`
...
* **[architecture.md](../docs/architecture.md) — every service, the network and
  volumes, the three authentication mechanisms, the request flows, and a table of
  what each failure actually means.
* **[configuration.md](../docs/configuration.md) — every `.env` variable and
