...
- **v2.9.0** (2026-09-18) — **the Windows packs know which Windows.** `req.target-min-winverclnt` is populated on 52,506 of sdk-api's 65,908 pages and 11,275 of the driver reference's, spanning Windows 2000 Professional to Windows 11 24H2 and Server 2025 — and the adapter parsed those keys and dropped them, so every pack built before this knew an API's header and its `.lib` and not whether the target machine exports the function at all. The OS fields now lead the contract, and the driver reference gains `req.kmdf-ver` / `req.umdf-ver` because a WDF page states a framework release and no OS at all. Rebuilt: **win32 118,242 symbols, 97,175 of them carrying a floor**, `wdk` 18,880. A grounded 52-question evaluation goes **7/52 → 51/52** against a rebuild of the same pages, with the `control-header` arm steady at 6/6 so the added line cost the existing answers nothing
- **v2.9.0** (2026-09-18) — **and one defect the evaluation found rather than the version work.** 45% of the reference is COM methods, titled `IFoo::Bar (header.h)` — a qualified name with no kind word, which the title regex required. The UID spells the same entity with a dot, so only that was indexed and `docs_lookup("IFoo::Bar")` answered *nothing*: 29,557 symbols unreachable under the name Microsoft documents and every compiler error quotes. Silence is the worst shape a miss can take here, because the server's instructions read an empty result as "undocumented" and forbid answering from memory. Both spellings are indexed now, which is most of win32's growth from 87,206 symbols to 118,242
- **v2.9.0** (2026-09-18) — **two new packs, and the console can manage them.** `win32-samples` (136.2 MB) and `wdk-samples` (76.8 MB) are the sample corpora the composites always advertised and neither shipped pack contained: what calling an API looks like in a working program, against the reference's description of what it does. The set is now 11 packs, 1.87 GB, 444,058 symbols, 0 unresolved each. The admin console gains a **Knowledge packs** page — installed packs with version, model, size and licence, plus install from a URL, update from a published index, and remove. Install and update run as jobs in Argus because a pack is up to a gigabyte and the console's client gives up after ten seconds; remove is one unlink and is not a job. Update needs `ARGUS_PACK_INDEX_URL`, and without it the button is absent and the card names the variable rather than failing
- **v2.9.0** (2026-09-18) — the README rewritten around the two halves the repository actually has, each with its own H1 — the stack, and Argus — after twelve releases of drift, with every number re-measured rather than carried over
