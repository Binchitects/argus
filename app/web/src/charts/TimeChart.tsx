...
import { cssVar, seriesColors } from './palette'
 ...
 * tooltip, and a legend whenever there is more than one series.
 */
export function TimeChart({ series: raw, unit, stacked, bars, height = 240, label, from, to }: {
  series: ChartSeries[]
  /** The selected range, epoch ms: the axis spans it even where there is no data, as in Grafana. */
  from?: number
  to?: number
  unit?: string
  stacked?: boolean
  ...
  useEffect(() => {
    ...
    const text = cssVar('--muted', '#62656d')
    const grid = cssVar('--border', '#e2e3e7')
    ...
    chart.setOption({
      ...
      tooltip: {
        ...
        borderColor: grid,
        textStyle: { color: cssVar('--text', '#1b1c1f') },
      },
      ...
      yAxis: {
        type: 'value',
        axisLabel: { color: text, formatter: (v: number) => formatValue(v, unit) },
      ...
      series: series.map((s) => ({
        ...
        itemStyle: bars ? { borderColor: surface, borderWidth: stacked ? 1 : 0 } : undefined,
  ...
  }, [series, unit, stacked, bars, from, to])

  return <div ref={ref} style={{ height }} role="img" aria-label={label} className="chart" />
}
